...
def main():
    ...
    # Read the reference image of a can without dents
    reference_image = cv2.imread("can1.jpg", cv2.IMREAD_GRAYSCALE)
    # Preprocess the reference image
    _, reference_binary = cv2.threshold(reference_image, 235, 255, cv2.THRESH_BINARY_INV)
    ...
    # Assuming there's only one contour in the reference image, use it as the reference contour
